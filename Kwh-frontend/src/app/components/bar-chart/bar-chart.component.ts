...
export class BarChartComponent implements OnInit, AfterViewInit {
  ...
  setLastMonth(): void {
    ...
    const month = (today.getMonth() + 1).toString().padStart(2, '0'); // Format to 'MM'
  ...
  fetchTenants(): void {
    this.http.get<any[]>(`${this.apiBaseUrl}/get-all-tenants`).subscribe({
      next: (data) => {
        this.tenants = data.filter(tenant => tenant.isDeleted == false && tenant.id !== 1); // Only include active tenants
      },
      error: (err) => {
