...
public class MonthlyKWHGenerationService {
    ...
    public List<MonthlyKwhReportDTO> generateManualKwhReport(List<String> tableNames, String fromDate, String toDate) {
        List<MonthlyKwhReportDTO> manualReports = new ArrayList<>();

        for (String tableName : tableNames) {
            String sql = String.format(
                    "SELECT " +
                            "  (SELECT TOP 1 kwh FROM %s WHERE timestamp >= ? ORDER BY timestamp ASC) AS start_kwh, " +
                            "  (SELECT TOP 1 kwh FROM %s WHERE timestamp <= ? ORDER BY timestamp DESC) AS end_kwh",
                    tableName, tableName
            );

            Map<String, Object> row = jdbcTemplate.queryForMap(sql, fromDate, toDate);
            ...
            if (startKwh != null && endKwh != null && endKwh >= startKwh) {
                double usage = endKwh - startKwh;
                BigDecimal rounded = BigDecimal.valueOf(usage).setScale(2, RoundingMode.HALF_UP);
                manualReports.add(new MonthlyKwhReportDTO(fromDate + " to " + toDate, rounded.doubleValue(), tableName));
            }
        }
